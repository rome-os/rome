...
| `CommandInput` paints no focus edge | Its field is `Input`'s `plain` variant, which keeps the edge; `CommandInput` turns it off. The row is a full-bleed header inside `Command`'s `overflow-hidden`, so an edge is clipped on three sides, and cmdk holds focus in the field for the life of the surface, so an edge keyed to it stays lit and marks nothing. The row's bottom rule is what reads the header against the list. |
