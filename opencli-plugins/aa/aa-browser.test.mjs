...
import { AA_HOME } from "./aa-form.mjs";
...
test("navigates fresh and waits for a stable complete result set", async () => {
  ...
  assert.deepEqual(await loadAaFlights(p, searchFor(), { now: p.now, submit: async () => {} }), d);
  assert.deepEqual(p.urls, [AA_HOME]);
...
for (const [key, pattern] of [
  ...
  test(`fails on ${key}`, async () => {
    ...
    await assert.rejects(
      loadAaFlights(p, searchFor(), { now: p.now, submit: async () => {} }),
      pattern,
    );
  });
test("login errors are distinguishable and never trigger cash fallback", async () => {
  ...
  await assert.rejects(
    loadAaFlights(p, searchFor("award"), { now: p.now, submit: async () => {} }),
...
test("remembers the highest expected count across a count regression", async () => {
  ...
  await assert.rejects(
    loadAaFlights(p, searchFor("cash", { timeout: 5 }), { now: p.now, submit: async () => {} }),
    /No partial/,
  );
});
...
test("waits for date hydration even when the grid is mounted and complete", async () => {
  ...
  assert.deepEqual(
    await loadAaFlights(p, searchFor("round-award"), { now: p.now, submit: async () => {} }),
    d,
  );
...
test("retries transient navigation context destruction, not arbitrary evaluate failures", async () => {
  ...
  assert.equal(
    (await loadAaFlights(p, searchFor(), { now: p.now, submit: async () => {} })).rows.length,
    5,
  );
  p.evaluate = async () => {
...
test("submission must finish before results are read", async () => {
  const p = mockPage([fixture()]);
  p.evaluate = async () => assert.fail("must not read stale results");
  await assert.rejects(
    loadAaFlights(p, searchFor(), {
      now: p.now,
      submit: async () => {
