...
The command opens AA's homepage and activates its own browser tab.
...
| `--timeout N` | Deadline for each of form setup and result loading, from 5–180 seconds. Default: 90 |
...
`search_url` is the result URL generated by AA. It can expire with the search session.
...
Homepage tests cover cash and awards, one-way and round-trip searches, hidden duplicates, disabled dates, exact airports, adult counts, and failed calendar dismissal.
