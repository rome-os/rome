...
The command opens United's homepage and fills the flight search widgets.
It selects the trip type and cash/miles mode, chooses exact airport suggestions, sets travelers, and picks dates from the calendar.
It submits **Find flights** and reads the resulting page. It does not construct a search URL.
The `url` output field contains the results URL generated by United.
...
| `--timeout 5..180` | Seconds allowed for form setup, then separately for all results to load. Defaults to 90. |
...
The search form uses Economy with flexible dates off and includes Basic economy cash fares.
Cabin, stop, duration, and price filters apply after all displayed flights load.
...
The form setup replaces remembered search settings and resets travelers before setting the requested adult count.
It does not return saved traveler identities.
